...
def create_common_parser():
    ...
    config_group.add_argument('--spacing',
                        type=str,
                        default='6',
                        help='Global spacing for lines/dots/tiles. '
...
def configure_util_parser(subparsers):
    ...
    # --- util list-templates ---
    list_tpl_parser = util_subparsers.add_parser('list-templates', help='List all available templates and title patterns and exit')
    list_tpl_parser.set_defaults(func=handle_list_templates)

    # --- util set-default-device ---
    set_dev_parser = util_subparsers.add_parser('set-default-device', help='Set the default device for future runs')
    ...
    set_margin_parser = util_subparsers.add_parser('set-default-margin', help='Set the default margin in mm for future runs')
    set_margin_parser.add_argument('margin_mm', type=float, metavar='MM', help='Margin in mm to set as default')
    ...
    info_parser.add_argument('spacing', type=str, metavar='SPACING', help='Spacing to analyze (e..g., "6mm" or "71px")')
    info_parser.add_argument('--device', choices=list_devices(), help='Target device (optional, uses default if not set)')
...
def configure_title_parser(subparsers, common_parser):
    ...
    # --- Common Style Args ---
    title_parser.add_argument('--line_width_px',
                       type=float,
                       default=0.5,
...
def configure_multi_parser(subparsers, common_parser):
    ...
    style_group.add_argument('--line_width_px', type=float, default=0.5, help='Line width (for lined, grid, etc.)')
    style_group.add_argument('--dot_radius_px', type=float, default=1.5, help='Dot radius (for dotgrid)')
    style_group.add_argument('--major_every', type=int, help='Make every Nth line thicker (for grid, lined)')
    style_group.add_argument('--major_width_add_px', type=float, default=1.5, help='Added width for major lines')
    style_group.add_argument('--crosshair_size', type=int, default=4, help='Size of cross-hairs (for grid)')
    style_group.add_argument('--no_crosshairs', action='store_true', help='Disable cross-hairs')
    style_group.add_argument('--midline_style', choices=['dashed', 'dotted'], default='dashed', help='Style for manuscript midline')
    style_group.add_argument('--ascender_opacity', type=float, default=0.3, help='Opacity for manuscript ascender line')
    style_group.add_argument('--staff_gap_mm', type=float, default=10, help='Gap between music staves')
    style_group.add_argument('--split-ratio', type=float, default=0.6, help='Split ratio for hybrid templates (default: 0.6)')
    style_group.add_argument('--section_gap_mm', type=float, help='Gap between sections in hybrid templates in mm')
    ...
    num_group.add_argument('--line-numbers-side', choices=['left', 'right'], default='left', help="Side (default: left)")
    num_group.add_argument('--line-numbers-interval', type=int, default=5, dest='line_numbers_interval_val', help="Number every Nth line (default: 5)")
    num_group.add_argument('--line-numbers-margin-px', type=int, default=40, help="Distance from page edge in pixels (default: 40)")
    num_group.add_argument('--line-numbers-font-size', type=int, default=18, help="Font size (default: 18)")
    num_group.add_argument('--line-numbers-grey', type=int, default=8, help="Greyscale level 0-15 (default: 8 = #808080)")

    # --- Cell Labeling Group (for child 'grid') ---
    cell_label_group = multi_parser.add_argument_group('Cell Labeling (for child "grid" cells)')
    cell_label_group.add_argument('--cell-labels', action='store_true', help="Enable 'A, B, C...' style labeling.")
    cell_label_group.add_argument('--cell-labels-y-side', choices=['left', 'right'], default='left', help="Side for Y-axis labels ('1, 2, 3...') (default: left)")
    cell_label_group.add_argument('--cell-labels-y-padding-px', type=int, default=10, help="Padding from left/right grid edge (default: 10)")
    cell_label_group.add_argument('--cell-labels-x-side', choices=['top', 'bottom'], default='bottom', help="Side for X-axis labels ('A, B, C...') (default: bottom)")
    cell_label_group.add_argument('--cell-labels-x-padding-px', type=int, default=10, help="Padding from top/bottom grid edge (default: 10)")
    cell_label_group.add_argument('--cell-labels-font-size', type=int, default=16, help="Font size (default: 16)")
    cell_label_group.add_argument('--cell-labels-grey', type=int, default=10, help="Greyscale level 0-15 (default: 10 = #a0a0a0)")

    # --- Axis Labeling Group (for child 'grid') ---
    axis_label_group = multi_parser.add_argument_group('Axis Labeling (for child "grid" cells)')
    axis_label_group.add_argument('--axis-labels', action='store_true', help="Enable '0, 5, 10...' style axis plot numbering.")
    axis_label_group.add_argument('--axis-labels-origin', choices=['topLeft', 'bottomLeft'], default='topLeft', help="Set the (0,0) origin (default: topLeft)")
    axis_label_group.add_argument('--axis-labels-interval', type=int, default=5, help="Number every Nth grid line (default: 5)")
    axis_label_group.add_argument('--axis-labels-y-side', choices=['left', 'right'], default='left', help="Side for Y-axis numbers (default: left)")
    axis_label_group.add_argument('--axis-labels-y-padding-px', type=int, default=10, help="Padding from left/right grid edge (default: 10)")
    axis_label_group.add_argument('--axis-labels-x-side', choices=['top', 'bottom'], default='bottom', help="Side for X-axis numbers (default: bottom)")
    axis_label_group.add_argument('--axis-labels-x-padding-px', type=int, default=10, help="Padding from top/bottom grid edge (default: 10)")
    axis_label_group.add_argument('--axis-labels-font-size', type=int, default=16, help="Font size (default: 16)")
    axis_label_group.add_argument('--axis-labels-grey', type=int, default=10, help="Greyscale level 0-15 (default: 10 = #a0a0a0)")
...
def configure_template_parsers(subparsers, common_parser):
    ...
    spec_group.add_argument('--line_width_px', type=float, default=0.5, help='Line width in pixels (default: 0.5)')
    spec_group.add_argument('--major_every', type=int, help='Make every Nth line thicker')
    spec_group.add_argument('--major_width_add_px', type=float, default=1.5, help='Added width for major lines')
    ...
    spec_group.add_argument('--dot_radius_px', type=float, default=1.5, help='Dot radius in pixels (default: 1.5)')
    spec_group.add_argument('--major_every', type=int, help='Make every Nth dot/line a crosshair')
    spec_group.add_argument('--crosshair_size', type=int, default=4, help='Size of cross-hair extensions in pixels (default: 4)')
    spec_group.add_argument('--force-major-alignment', action='store_true', help='Force grid to end on major lines by adjusting margins')
    ...
    spec_group.add_argument('--line_width_px', type=float, default=0.5, help='Line width in pixels (default: 0.5)')
    spec_group.add_argument('--major_every', type=int, help='Make every Nth line thicker')
    spec_group.add_argument('--major_width_add_px', type=float, default=1.5, help='Added width for major lines (default: 1.5)')
    spec_group.add_argument('--crosshair_size', type=int, default=3, help='Size of cross-hair extensions in pixels (default: 3)')
    spec_group.add_argument('--no_crosshairs', action='store_true', help='Disable cross-hairs at major intersections')
    spec_group.add_argument('--force-major-alignment', action='store_true', help='Force grid to end on major lines by adjusting margins')
    ...
    label_group.add_argument('--axis-labels', action='store_true', help="Enable '0, 5, 10...' style axis numbering")
    ...
    cell_label_group.add_argument('--cell-labels-x-padding-px', type=int, default=10, help="Padding from top/bottom grid edge (default: 10)")
    cell_label_group.add_argument('--cell-labels-font-size', type=int, default=16, help="Font size (default: 16)")
    cell_label_group.add_argument('--cell-labels-grey', type=int, default=10, help="Greyscale level 0-15 (default: 10)")
    ...
    axis_label_group.add_argument('--axis-labels-x-padding-px', type=int, default=10, help="Padding from top/bottom grid edge (default: 10)")
    axis_label_group.add_argument('--axis-labels-font-size', type=int, default=16, help="Font size (default: 16)")
    axis_label_group.add_argument('--axis-labels-grey', type=int, default=10, help="Greyscale level 0-15 (default: 10)")
    ...
    spec_group.add_argument('--line_width_px', type=float, default=0.5, help='Line width in pixels (default: 0.5)')
    spec_group.add_argument('--midline_style', choices=['dashed', 'dotted'], default='dashed', help='Style for manuscript midline (default: dashed)')
    spec_group.add_argument('--ascender_opacity', type=float, default=0.3, help='Opacity for manuscript ascender line (default: 0.3)')
    ...
    spec_group.add_argument('--line_width_px', type=float, default=0.5, help='Line width in pixels (default: 0.5)')
    spec_group.add_argument('--staff_gap_mm', type=float, default=10, help='Gap between music staves in mm (default: 10)')
    ...
    hybrid_parser.set_defaults(func=handle_single_template_generation, template_type='hybrid_lined_dotgrid')
    
    spec_group = hybrid_parser.add_argument_group('Hybrid Template Options')
    spec_group.add_argument('--line_width_px', type=float, default=0.5, help='Line width for lined section (default: 0.5)')
    spec_group.add_argument('--dot_radius_px', type=float, default=1.5, help='Dot radius for dotgrid section (default: 1.5)')
    spec_group.add_argument('--split-ratio', type=float, default=0.6, help='Split ratio (default: 0.6)')
    spec_group.add_argument('--section_gap_mm', type=float, help='Gap between sections in mm (defaults to same as --spacing)')
