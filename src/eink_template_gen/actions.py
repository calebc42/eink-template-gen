...
from .config import set_default_device, get_default_device, set_default_margin, get_default_margin
from .utils import (
    generate_filename, 
    parse_spacing, 
...
def _setup_generation_context(args):
    ...
    if not device_id:
        ...
        if not device_id:
            raise ValueError("No device specified and no default device set. Use --device DEVICE or set a default.")
    ...
    if context['using_line_count_mode']:
        ...
        # --- START FIX for KeyError: 'spacing_mm_to_use' ---
...
def _build_template_kwargs(template_type, args):
    ...
    # Common
    if cli_args.get('line_width_px') is not None: kwargs['line_width_px'] = cli_args['line_width_px']
    if cli_args.get('dot_radius_px') is not None: kwargs['dot_radius_px'] = cli_args['dot_radius_px']
    
    # Grid/Lined features
    if cli_args.get('major_every') is not None:
        kwargs['major_every'] = cli_args['major_every']
        
        # --- START FIX for KeyError: 'major_width_add_px' ---
    ...
    # Grid features
    if cli_args.get('crosshair_size') is not None: kwargs['crosshair_size'] = cli_args['crosshair_size']
    if cli_args.get('no_crosshairs') is not None: kwargs['no_crosshairs'] = cli_args['no_crosshairs']
    
    # Manuscript
    if cli_args.get('midline_style') is not None: kwargs['midline_style'] = cli_args['midline_style']
    if cli_args.get('ascender_opacity') is not None: kwargs['ascender_opacity'] = cli_args['ascender_opacity']
    
    # Music
    if cli_args.get('staff_gap_mm') is not None: kwargs['staff_gap_mm'] = cli_args['staff_gap_mm']
    
    # Hybrid
    if cli_args.get('split_ratio') is not None: kwargs['split_ratio'] = cli_args['split_ratio']
    if cli_args.get('section_gap_mm') is not None: kwargs['section_gap_mm'] = cli_args['section_gap_mm']
...
def _save_and_print_summary(surface, context, args):
    ...
    if args.command == 'multi':
        if args.template:
            ...
            print(f"  - Template: Multi-Type Grid")
    ...
    if context['using_line_count_mode']:
        spacing_display = format_line_count_summary(
            context['h_lines'], context['v_lines'], 
            context['h_spacing_px'], context['v_spacing_px'], 
        ...
        spacing_display = format_spacing_summary(
            context['spacing_px'], context['original_mm'], 
            context['adjusted_mm'], context['was_adjusted'], 
        ...
        if not is_complex_layout:
            ...
            if force_align:
                ...
                m_left, m_right, _ = calculate_major_aligned_margins_x(content_width, h_align_unit, base_margin_px, cli_args['major_every'])
            else:
                m_top, m_bottom = calculate_adjusted_margins(content_height, v_align_unit, base_margin_px)
                m_left, m_right = calculate_adjusted_margins_x(content_width, h_align_unit, base_margin_px)

            margin_adjusted = (abs(m_top - base_margin_px) > 0.5 or abs(m_left - base_margin_px) > 0.5)

            if force_align:
                 print(f"  - Margin: {margin_mm}mm (adjusted for major alignment: "
                       f"T:{m_top/mm2px:.2f}, B:{m_bottom/mm2px:.2f}, L:{m_left/mm2px:.2f}, R:{m_right/mm2px:.2f}mm)")
            elif margin_adjusted:
                print(f"  - Margin: {margin_mm}mm (adjusted for pixel-perfect: "
                      f"T:{m_top/mm2px:.2f}, B:{m_bottom/mm2px:.2f}, L:{m_left/mm2px:.2f}, R:{m_right/mm2px:.2f}mm)")
            else:
...
def handle_cover_generation(args):
    ...
    if args.title in ['hilbert_curve', 'dragon_curve', 'koch_snowflake', 'plant_fractal', 'sierpinski_triangle']:
...
def handle_multi_template_generation(args):
    ...
    if args.cell_types:
        # --- Mixed-Type Grid ---
        print(f"Generating {num_rows}x{num_columns} multi-type grid for {context['device_config']['name']}...")
        template_func = create_cell_grid_template
        
        cell_type_list = args.cell_types.split(',')
        if len(cell_type_list) != (num_columns * num_rows):
            raise ValueError(f"--cell_types list has {len(cell_type_list)} items, but grid is {num_rows}x{num_columns}")
        ...
        print(f"Generating {num_rows}x{num_columns} uniform '{template_type}' grid for {context['device_config']['name']}...")
